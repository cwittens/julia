...
function require_stdlib(uuidkey::PkgId)
    @lock require_lock begin
    if root_module_exists(uuidkey)
...
@constprop :none function stale_cachefile(modkey::PkgId, build_id::UInt128, modpath::String, cachefile::String;
                                          ignore_loaded::Bool = false, reasons::Union{Dict{String,Int},Nothing}=nothing, stalecheck::Bool=true)
    # XXX: this function does all of the file validation, not just those checks related to stale
    io = open(cachefile, "r")
    try
