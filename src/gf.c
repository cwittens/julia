        ...
        jl_method_instance_t *mi JL_PROPAGATES_ROOT, jl_code_info_t *src)
{
    ...
    if (max_world >= jl_atomic_load_acquire(&jl_world_counter))
        max_world = ~(size_t)0;
    return jl_get_method_inferred(compiler, mi, src->rettype, src->min_world, max_world);
}
...
JL_DLLEXPORT void jl_set_typeinf_func(jl_value_t *f)
{
    ...
    if (newfunc) {
        ...
        for (i = 0, l = jl_array_nrows(unspec); i < l; i++) {
            ...
            if (jl_rettype_inferred(compiler, mi, world, world) == jl_nothing)
                jl_type_infer(compiler, mi, world, 1);
        }
        JL_GC_POP();
    }
}
...
static void _generate_from_hint(jl_method_instance_t *mi, size_t world)
{
    ...
    if (codeinst == jl_nothing) {
        (void)jl_type_infer(compiler, mi, world, 1);
    ...
    if (jl_generating_output()) {
        ...
        if (types && !jl_subtype(mi->specTypes, (jl_value_t*)types)) {
            ...
            if (jl_rettype_inferred(compiler, mi2, world, world) == jl_nothing)
                (void)jl_type_infer(compiler, mi2, world, 1);
            if (jl_typeinf_func &&  jl_atomic_load_relaxed(&mi->def.method->primary_world) <= tworld) {
                if (jl_rettype_inferred(compiler, mi2, tworld, tworld) == jl_nothing)
                    (void)jl_type_infer(compiler, mi2, tworld, 1);
            }
        }
    }
    else {
